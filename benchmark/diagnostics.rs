...
fn npy_header(bytes: &[u8]) -> (Vec<usize>, usize) {
    assert!(bytes.len() >= 12, "truncated .npy header");
...
fn read_triples(path: &str) -> Vec<[f64; 9]> {
    ...
    assert_eq!(shape.len(), 3, "expected three dimensions, got {shape:?}");
    assert_eq!(&shape[1..], &[3, 3], "expected (N,3,3), got {shape:?}");
    assert!(shape[0] > 0, "the corpus must contain at least one row");
    assert_eq!(
        shape[0].checked_mul(72).and_then(|n| n.checked_add(off)),
        Some(bytes.len()),
        "wrong .npy payload length"
    );
    (0..shape[0])
        .map(|i| {
            ...
            for (k, slot) in row.iter_mut().enumerate() {
                ...
                *slot = f64::from_le_bytes(bytes[p..p + 8].try_into().unwrap());
                assert!(
                    slot.is_finite(),
                    "nonfinite coordinate at row {i}, slot {k}"
                );
            }
            row
        })
...
fn bench_row(path: &str, k: usize, reps: usize) -> bool {
    let triples = read_triples(path);
    init_tables();
    let r = triples[k];
...
fn bench_triple(values: &[f64]) -> bool {
...
fn main() {
    ...
    let valid_arity = match args.first().map(String::as_str) {
        Some("bench-paired" | "bench-npy" | "census-npy") => (2..=3).contains(&args.len()),
        Some("bench-row") => (3..=4).contains(&args.len()),
        Some("bench-triple") => args.len() == 10,
        _ => false,
    };
    if !valid_arity {
        eprintln!(
    ...
    let parse_index = |s: &String| -> usize {
        s.parse().unwrap_or_else(|_| {
            eprintln!("invalid nonnegative integer: {s}");
            std::process::exit(2);
        })
    };
    let parse = |s: &String| -> usize {
        let value = parse_index(s);
        if value == 0 {
            eprintln!("stride and repetition counts must be positive");
            std::process::exit(2);
        }
        value
    };
    let ok = match args.first().map(String::as_str) {
        ...
        Some("bench-row") => bench_row(
            &args[1],
            parse_index(&args[2]),
            args.get(3).map_or(1, parse),
        ),
        Some("bench-triple") => {
            let values: Vec<f64> = args[1..]
                ...
                .map(|v| {
                    v.parse::<f64>()
                        ...
                        .unwrap_or_else(|| {
                            eprintln!("invalid finite coordinate: {v}");
                            std::process::exit(2);
                        })
                })
                .collect();
